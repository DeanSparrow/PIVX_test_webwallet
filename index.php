...
if (!empty($_SESSION['user_session'])) {
    ...
    if (!empty($_POST['action'])) {
      switch ($_POST['action']) {
        ...
        case "logout":
          ...
          break;
        case "support":
          $info_pop['message'] = "Please contact support via email at $support </br> Support Key: " . $_SESSION['user_supportpin'];
          break;
        case "disauth":
          $user = new User($mysqli);
          $disauth = $user->disauth();
          $info_pop['message'] = $disauth;
          break;
        case "authgen":
          $user = new User($mysqli);
          $secret = $user->createSecret();
          $gen = $user->enableauth();
          $info_pop['message'] =   $gen;
          break;
      }
    }
  ...
  if ($response->success | $recaptcha_enable === false) {
    ...
    if (isset($_POST['action'])) {
      ...
      switch ($_POST['action']) {
        case "login":
          $result = $user->logIn($_POST['email'], $_POST['password'], $_POST['auth']);
          if (!is_array($result)) {
            $error['type'] = "login";
            $error['message'] = $result;
          } else {
          ...
          break;
        case "register":
          $result = $user->add($_POST['username'], $_POST['email'], $_POST['password'], $_POST['confirmPassword']);
          if ($result !== true) {
            $error['type'] = "register";
            $error['message'] = $result;
          } else {
